PROGRAM HALO_SPARSITY
  ...
  IMPLICIT NONE
  INTEGER :: I_SPARS
  REAL(8) :: REDSHIFT, SPARSITY_200500, SPARSITY_5001000
  ...
  WRITE(*,*) ' Insert Mass Ratio Overdensities'
  WRITE(*,*) ' s_{200,500} (1) s_{500,1000} (2)'
  READ(*,*) I_SPARS

  WRITE(*,*) 'Select Mass Function (Despali/Raygal)'
  READ(*,*) select_mf_option
  ...
  IF(I_SPARS.EQ.1) THEN
     write(*,'(a2,f5.3,a3,f9.5)') 's(',REDSHIFT,') =',SPARSITY_200500(REDSHIFT)
  ELSE IF(I_SPARS.EQ.2) Then
     write(*,'(a2,f5.3,a3,f9.5)') 's(',REDSHIFT,') =',SPARSITY_5001000(REDSHIFT)
  END IF

END PROGRAM HALO_SPARSITY


FUNCTION SPARSITY_5001000(REDSHIFT)
  USE COSMO
  USE MF
  IMPLICIT NONE
  REAL(8) :: SPARSITY_TEMP, SPARSITY_5001000, REDSHIFT
  REAL(8) :: SPARSITY_DESPALI_CORR_5001000
...
200  CONTINUE
     ...
     CASE('Raygal')     
        SPARSITY_5001000=SPARSITY_TEMP
     CASE('Despali')
        SPARSITY_DESPALI_CORR_5001000 = 0.119859886 - 0.319620798 * REDSHIFT &
                & + 0.24493871 * REDSHIFT**2 -0.05015553 * REDSHIFT**3
        SPARSITY_5001000 = SPARSITY_TEMP - SPARSITY_DESPALI_CORR_5001000    
     END SELECT mf_selector

   END FUNCTION SPARSITY_5001000

   FUNCTION SPARSITY_200500(REDSHIFT)
     USE COSMO
     USE MF
     IMPLICIT NONE
     REAL(8) :: SPARSITY_TEMP, SPARSITY_200500, REDSHIFT
     REAL(8) :: SPARSITY_DESPALI_CORR_200500
     REAL(8) :: SD0_OLD, SD1_OLD, HS0_OLD, HS1_OLD
     REAL(8) :: CHANGE
     REAL(8), PARAMETER :: M1=1.d+13, M2=1.d+16
     INTEGER :: ITER
     ...
     SD0_OLD=1.2d0
     SD1_OLD=1.3d0
    HS0_OLD=SD0_OLD*MF_M200c_INTEGRAL(SD0_OLD*M1,SD0_OLD*M2,REDSHIFT)- &
         & MF_M500c_INTEGRAL(M1,M2,REDSHIFT)
    HS1_OLD=SD1_OLD*MF_M200c_INTEGRAL(SD1_OLD*M1,SD1_OLD*M2,REDSHIFT)- &
         & MF_M500c_INTEGRAL(M1,M2,REDSHIFT)
...
200     CONTINUE
        ...
        mf_selector: SELECT CASE(select_mf_option)
        CASE('Raygal')
           SPARSITY_200500 = SPARSITY_TEMP
        CASE('Despali')
           SPARSITY_DESPALI_CORR_200500 = 0.14073268 - 0.37056373*REDSHIFT &
                & + 0.32410084 * REDSHIFT**2 - 0.07044901 * REDSHIFT**3
           SPARSITY_200500 = SPARSITY_TEMP - SPARSITY_DESPALI_CORR_200500 
        END SELECT mf_selector

      END FUNCTION SPARSITY_200500     
